...
{
    /// <summary>
    /// Translates SCIM FilterExpression AST to LINQ Expression trees for ScimUser.
    /// This allows filters to be executed server-side (e.g., in EF Core queries).
    /// </summary>
    public class ScimUserFilterTranslator : IScimFilterTranslator<ScimUser>
    {
        /// <summary>
        /// Converts a SCIM FilterExpression to a LINQ Expression predicate.
        /// </summary>
        public Expression<Func<ScimUser, bool>>? BuildPredicate(FilterExpression? filter)
        {
            if (filter == null)
                return null;
            ...
            return Expression.Lambda<Func<ScimUser, bool>>(body, parameter);
        }

        /// <summary>
        /// Applies a SCIM filter to an IQueryable source.
        /// </summary>
        public IQueryable<ScimUser> Apply(IQueryable<ScimUser> source, FilterExpression? filter)
        {
        ...
        /// <summary>
        /// Gets property access expression from attribute path (supports nested properties like "name.givenName").
        /// </summary>
        ...
        private Expression BuildEqualsExpression(Expression property, Expression value, object rawValue)
        {
            ...
            {
                // Convert bool to nullable bool for comparison
